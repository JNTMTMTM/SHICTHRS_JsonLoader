...
def encrypt_dict_values(data_dict : dict , key : str) -> dict:
    """递归地对字典中的所有值进行加密
    Args:
        data_dict: 要加密的字典
        key: 加密密钥
    ...
    for key_name, value in data_dict.items():
        if isinstance(value, dict):
            # 如果值是字典，递归处理
            encrypted_dict[key_name] = encrypt_dict_values(value, key)
        elif isinstance(value, (list, tuple)):
            # 如果值是列表或元组，处理每个元素
            encrypted_list = []
            for item in value:
                if isinstance(item, dict):
                    encrypted_list.append(encrypt_dict_values(item, key))
                elif isinstance(item, str):
                    encrypted_list.append(encrypt_with_key(item, key))
                else:
                    # 非字符串类型转换为字符串后再加密
                    encrypted_list.append(encrypt_with_key(str(item), key))
            encrypted_dict[key_name] = encrypted_list
        elif isinstance(value, str):
            # 字符串值直接加密
            encrypted_dict[key_name] = encrypt_with_key(value, key)
        else:
            # 非字符串类型转换为字符串后再加密
            encrypted_dict[key_name] = encrypt_with_key(str(value), key)
...
def write_json_file(json_dict : dict , path : str , ectype : str , key : str) -> None:
    if ectype == 'b4':
        ...
        # 深拷贝原始字典，避免修改原始数据
        encrypted_dict = encrypt_dict_values(deepcopy(json_dict), key)
        # 将加密后的字典写入文件
